%This function is essencially the NPAhiarchy from QETLAB, the only
%difference is that I have impose the probabilities to sum to a number that
%is smaller then or equals 1, not to be exactly one. This allows us to work
%with unormalised (but still positive) distributions.
...
function is_npa = NPAHierarchy1(p,varargin)
                ...
                if(k >= 1)
                    ...
                    for i = 1:real_dim
                        for j = i:real_dim
                            ...
                            elseif(res_type == 1)
                                if(isequal(res,[0;-1]))
                                    G(i,j) == sum(sum(p,1),2); % identity measurement %Mirdit Comment ==1 
